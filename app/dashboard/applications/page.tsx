  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      const { data, error } = await supabase
        .from("applications")
        .select("*, jobs(id, title, location, companies(name))")
                ...
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-foreground mb-1">{application.jobs?.title}</h3>
                      <p className="text-sm text-muted-foreground mb-3">{application.jobs?.companies?.name}</p>
                    ...
                    <Badge className={`capitalize ${statusColors[application.status as keyof typeof statusColors]}`}>
                      {application.status}
                    </Badge>
                  </div>
                </CardContent>
